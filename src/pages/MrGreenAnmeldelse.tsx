...
const mrgreenFaqs: { question: string; answer: ReactNode }[] = [
  ...
  { question: "Hvem ejer Mr Green Casino?", answer: "Mr Green blev grundlagt i Sverige i 2007 og opkøbt af William Hill i 2019. I 2021 fusionerede William Hill med 888 Holdings (nu Evoke PLC), som i dag er den overordnede modergruppe. Denne koncernstruktur giver Mr Green adgang til massive ressourcer inden for teknologi, sikkerhed og spiludvikling, samtidig med at brandet bevarer sin unikke identitet og designfilosofi. Mr Green tilbyder casino, sportsbetting og live casino under dansk licens." },
];

const MrGreenAnmeldelse = () => {
  const { data: siteSettings } = useSiteSettings();
  const heroBackgroundImage = siteSettings?.hero_background_image;
  const articleSchema = buildArticleSchema({ headline: "Mr Green Casino Anmeldelse 2026 – Dansk Licens, Bonus & Ansvarligt Spil", description: "Komplet anmeldelse af Mr Green Casino. Prisbelønnet platform med dansk licens, Green Gaming og kurateret spiludvalg.", url: "https://casinoaftaler.dk/casino-anmeldelser/mr-green", datePublished: "2026-02-15", dateModified: "2026-02-17", authorName: "Jonas", authorUrl: "https://casinoaftaler.dk/forfatter/jonas", aggregateRating: { ratingValue: "4.3", ratingCount: "219" } });
    ...
    <>
      ...
      <section className="relative overflow-hidden py-12 text-white md:py-20" style={{ backgroundImage: heroBackgroundImage ? `linear-gradient(135deg, hsl(260 70% 25% / 0.95), hsl(210 80% 30% / 0.9)), url(${heroBackgroundImage})` : "linear-gradient(135deg, hsl(260 70% 25%), hsl(250 60% 20%) 40%, hsl(210 80% 25%))", backgroundSize: "cover", backgroundPosition: "center" }}>
        <div className="container"><div className="mx-auto max-w-3xl text-center">
          <Badge variant="secondary" className="mb-4"><Award className="mr-1.5 h-3.5 w-3.5" />4.3 / 5 – Prisbelønnet Casino</Badge>
      ...
      <div className="container py-8 md:py-12">
        ...
        <section className="mb-12"><Card className="border-border bg-card border-l-4 border-l-primary"><CardHeader><CardTitle className="flex items-center gap-2 text-xl"><Zap className="h-6 w-6 text-primary" />Hurtige Fakta – Mr Green Casino</CardTitle></CardHeader><CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div className="rounded-lg border border-border p-3"><p className="text-xs text-muted-foreground uppercase mb-1">Velkomstbonus</p><p className="text-lg font-bold text-foreground">100 kr. kontant + 50 FS</p></div>
            <div className="rounded-lg border border-border p-3"><p className="text-xs text-muted-foreground uppercase mb-1">Omsætningskrav</p><p className="text-lg font-bold text-foreground">Ingen (CASH-kode)</p></div>
            ...
            <div className="rounded-lg border border-border p-3"><p className="text-xs text-muted-foreground uppercase mb-1">Grundlagt</p><p className="text-lg font-bold text-foreground">2007</p></div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-center mt-4">
            ...
            <div className="rounded-lg border border-border p-3"><p className="text-xs text-muted-foreground uppercase mb-1">Udbetaling</p><p className="text-lg font-bold text-foreground">1–5 dage</p></div>
            <div className="rounded-lg border border-border p-3"><p className="text-xs text-muted-foreground uppercase mb-1">Ejer</p><p className="text-lg font-bold text-foreground">888 Holdings / Evoke PLC</p></div>
          </div>
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="border-border bg-card"><CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-lg text-primary"><Check className="h-5 w-5" />Fordele</CardTitle></CardHeader><CardContent><ul className="space-y-2">{["Prisbelønnet design og brugeroplevelse – æstetisk i topklasse", "Innovativt Green Gaming ansvarligt spil-værktøj med AI-analyse", "Kurateret spiludvalg af konsekvent høj kvalitet", "Dansk licens fra Spillemyndigheden", "Casino, sportsbetting og live casino samlet under ét brand", "Dedikeret mobilapp (iOS/Android) med flere branchepriser", "Del af 888 Holdings (Evoke PLC) – børsnoteret med solid finansiel baggrund", "MobilePay, Trustly, Skrill, Neteller og Revolut understøttet", "Dedikerede Mr Green live casino-borde med lavere minimums", "50 free spins uden indbetaling + kontant velkomstbonus uden omsætningskrav"].map((p) => (<li key={p} className="flex items-start gap-2 text-sm"><Check className="h-4 w-4 text-emerald-500 flex-shrink-0 mt-0.5" /><span className="text-muted-foreground">{p}</span></li>))}</ul></CardContent></Card>
            <Card className="border-border bg-card"><CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-lg text-destructive/80"><X className="h-5 w-5" />Ulemper</CardTitle></CardHeader><CardContent><ul className="space-y-2">{["Spiludvalget (1.000+) er mindre end hos konkurrenter med 2.000+", "Udbetalinger kan tage 1–5 dage afhængigt af metode", "Live chat kun tilgængelig 09:00–21:00", "Ingen poker-sektion", "VIP-programmet mangler gennemsigtighed i niveaustruktur", "Sænket RTP på visse spilleautomater rapporteret af spillere"].map((c) => (<li key={c} className="flex items-start gap-2 text-sm"><X className="h-4 w-4 text-destructive flex-shrink-0 mt-0.5" /><span className="text-muted-foreground">{c}</span></li>))}</ul></CardContent></Card>
          </div>
        </section>
        ...
        <section className="mb-12">
          ...
          <p className="mb-4 text-muted-foreground leading-relaxed">Mr Green Casino tilbyder flere <Link to="/velkomstbonus" className={linkClass}>velkomsttilbud</Link> til nye spillere. Det primære tilbud er: indbetal 100 kr. med koden CASH og få 100 kr. kontant – helt <Link to="/bonus-uden-omsaetningskrav" className={linkClass}>uden omsætningskrav</Link>. Derudover tilbydes 50 <Link to="/free-spins" className={linkClass}>free spins</Link> på Legacy of Dead uden indbetaling. Der findes også et alternativt tilbud med koden CASINO: 100% matchbonus op til 1.000 kr. med 10x <Link to="/omsaetningskrav" className={linkClass}>omsætningskrav</Link>.</p>
          ...
          <p className="mb-4 text-muted-foreground leading-relaxed">CASH-tilbuddet (100 kr. kontant uden omsætning) er et af de mest fordelagtige på det danske marked – du får reelt 100 kr. i ren værdi. For CASINO-tilbuddet: indbetaler du 1.000 kr. og modtager 1.000 kr. i bonus, skal du gennemspille (1.000 + 1.000) × 10 = 20.000 kr. Ved en gennemsnitlig RTP på 96% er det statistisk forventede tab ~800 kr. – hvilket giver en teoretisk nettogevinst på ~200 kr. De 50 free spins uden indbetaling er ren ekstra værdi oven i hatten.</p>
        ...
        <section className="mb-12">
          ...
          <p className="mb-6 text-muted-foreground leading-relaxed">Mr Green Casino tilbyder et solidt udvalg af <Link to="/betalingsmetoder" className={linkClass}>betalingsmetoder</Link> for danske spillere inkl. <Link to="/betalingsmetoder/mobilepay" className={linkClass}>MobilePay</Link>, <Link to="/betalingsmetoder/trustly" className={linkClass}>Trustly</Link>, <Link to="/betalingsmetoder/skrill" className={linkClass}>Skrill</Link>, Neteller, <Link to="/betalingsmetoder/revolut" className={linkClass}>Revolut</Link>, <Link to="/betalingsmetoder/visa-mastercard" className={linkClass}>Visa/Mastercard</Link> og bankoverførsel. Min. indbetaling er 50 kr., og alle transaktioner er gebyrfri.</p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            {[
              { title: "MobilePay", desc: "Hurtig og nem indbetaling direkte fra mobilen. Den mest populære metode hos danske spillere.", speed: "⚡ Øjeblikkeligt" },
              { title: "Trustly", desc: "Bankbaseret betaling med hurtige ind- og udbetalinger. Udbetalinger typisk inden for 1-5 dage.", speed: "🕐 1-5 dage" },
              { title: "Visa / Mastercard", desc: "Klassiske kortbetalinger med øjeblikkelig indbetaling. Udbetalinger inden for 1-5 dage.", speed: "🕐 1-5 dage" },
              { title: "Skrill / Neteller / Revolut", desc: "E-wallets med hurtige udbetalinger – typisk 1-3 dage.", speed: "⚡ 1-3 dage" },
            ].map((m) => (
        ...
        <section className="mb-12">
          ...
          <p className="mb-4 text-muted-foreground leading-relaxed">Mr Green tilbyder kundeservice via live chat, e-mail og telefon. Live chatten er tilgængelig fra 09:00 til 21:00 CET, hvilket er en ulempe sammenlignet med operatører, der tilbyder 24/7 support. Uden for åbningstiden kan du sende en e-mail, der typisk besvares inden for 24 timer.</p>
