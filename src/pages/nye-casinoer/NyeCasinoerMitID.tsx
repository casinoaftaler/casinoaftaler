...
import { ShieldCheck, Sparkles, CheckCircle2, Smartphone } from "lucide-react";
...
const faqs: { question: string; answer: ReactNode }[] = [
  { question: "Hvad er MitID, og hvorfor kræves det på nye casinoer?", answer: "MitID er Danmarks digitale identitetsløsning, der erstattede NemID i 2023. Alle danske licenserede casinoer er lovmæssigt forpligtet til at verificere spilleres identitet via MitID som led i KYC-kravene (Know Your Customer). Det sikrer, at kun myndige danske borgere kan spille, og forhindrer identitetsmisbrug." },
  { question: "Hvor lang tid tager MitID-verifikation hos nye casinoer?", answer: (
    <>
      MitID-verifikation på nye casinoer tager typisk under 2 minutter. De bedste nye casinoer har integreret MitID direkte i registreringsprocessen, så verifikation sker i realtid. Det eliminerer den traditionelle ventetid på manuel dokumentgennemgang og giver dig adgang til at spille med det samme.
    </>
  )},
  { question: "Kan man oprette en casinokonto uden MitID?", answer: "Nej, alle danske licenserede casinoer kræver MitID-verifikation som betingelse for kontooprettelse. Det er et lovkrav fra Spillemyndigheden, der sikrer spillerbeskyttelse, ROFUS-tilslutning og forebyggelse af hvidvask. Casinoer uden MitID-krav opererer uden dansk licens." },
  { question: "Er mine data sikre, når jeg bruger MitID på nye casinoer?", answer: "Ja, MitID bruger bankgraderet kryptering og multi-faktor-autentificering. Casinoet modtager kun de nødvendige identifikationsdata – ikke dine bankoplysninger eller andre følsomme data. MitID er udviklet og driftet af Nets/Nexi under streng regulering fra Digitaliseringsstyrelsen." },
  { question: "Hvad hvis jeg har problemer med MitID på et nyt casino?", answer: (
    <>
      Kontakt først casinoets kundeservice. Hvis problemet er teknisk med MitID selv, kan du kontakte MitID-supporten. De fleste nye casinoer har integreret alternative verifikationsmetoder som fallback. Er du i tvivl om et casinos legitimitet, så tjek vores liste over <Link to="/nye-casinoer/dansk-licens" className={linkClass}>nye casinoer med dansk licens</Link>.
    </>
  )},
];
    ...
    <>
      ...
      <div className="container py-8 md:py-12">
        ...
        <section className="mb-12">
          ...
          <p className="text-muted-foreground leading-relaxed">MitID-verifikation er din garanti for, at casinoet opererer lovligt under dansk licens fra <Link to="/spillemyndigheden" className={linkClass}>Spillemyndigheden</Link>. Casinoer, der ikke kræver MitID, har ikke dansk licens og bør undgås.</p>
        </section>
        ...
        <section className="mb-12">
          ...
          <div className="space-y-3">
            {["Hurtig verifikation på under 2 minutter", "Automatisk ROFUS-tjek ved registrering", "Ingen manuelle dokumentuploads", "Bankgraderet sikkerhed med multi-faktor-autentificering", "Garanti for at casinoet har dansk licens", "Forhindrer identitetsmisbrug og mindreårige spillere"].map((b) => (
