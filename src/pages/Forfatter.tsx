...
export default function Forfatter() {
  ...
  const { data: newsData } = usePublishedNewsByAuthor("jonas", 1, 100);
  const jonasNews = newsData?.articles ?? [];
  ...
  return (
    <>
      ...
      <div className="container py-8 md:py-12">
        ...
        {/* Casino Nyheder af Jonas (dynamisk fra database) */}
        {jonasNews.length > 0 && (
          <>
            <Separator className="my-10" />
            <section className="mb-12">
              <h2 className="text-3xl font-bold flex items-center gap-2 mb-6">
                ...
                Casino Nyheder af Jonas
              </h2>
              <div className="grid gap-4 md:grid-cols-2">
                {jonasNews.map((article) => (
                  <Link
                    key={article.slug}
