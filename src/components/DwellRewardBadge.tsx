      ...
      >
        ...
        <div className="relative h-14 w-14 flex-shrink-0">
          <svg className="h-14 w-14 -rotate-90" viewBox="0 0 56 56">
            <circle cx="28" cy="28" r="26" fill="none" stroke="currentColor" strokeWidth="3" className="text-muted/20" />
            {!completed && (
              <circle cx="28" cy="28" r="26" fill="none" stroke="url(#dwell-gradient)" strokeWidth="3.5" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} className="transition-all duration-300" />
            )}
            {completed && (
              <circle cx="28" cy="28" r="26" fill="none" stroke="currentColor" strokeWidth="3" className="text-emerald-500" />
            )}
            <defs>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            {completed ? (
              ...
              <span className="text-base font-black tabular-nums text-foreground">{secondsLeft}</span>
            )}
          </div>
        </div>
        ...
        <div className="relative flex flex-col gap-1">
          {completed ? (
            <>
              <span className="text-base font-bold text-emerald-600 dark:text-emerald-400">+300 credits! 🎉</span>
              {nextMission ? (
                <Link
                  to={nextMission.path}
