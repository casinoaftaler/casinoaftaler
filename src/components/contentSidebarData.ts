/**
 * Sidebar navigation data for content/money pages.
 * Follows the top menu structure: Casinoer, Nye Casinoer, Casinospil, Live Casino,
 * Casino Bonus, Betalingsmetoder, Spiludviklere, Casino Anmeldelser.
 */

export interface SidebarLink {
  to: string;
  label: string;
  iconName?: string;
}

export interface SidebarCategory {
  title: string;
  iconName: string;
  hubTo?: string; // link for collapsed "Se alle" / category header
  links: SidebarLink[];
}

export const SIDEBAR_CATEGORIES: SidebarCategory[] = [
  {
    title: "Casinoer",
    iconName: "crown",
    hubTo: "/top-10-casino-online",
    links: [
      { to: "/top-10-casino-online", label: "Top 10 Online Casino", iconName: "trophy" },
      { to: "/casinoer/hurtig-udbetaling", label: "Hurtig Udbetaling", iconName: "zap" },
      { to: "/casinoer/hoej-rtp", label: "Høj RTP", iconName: "trending-up" },
      { to: "/casino-licenser", label: "Licenserede Casinoer", iconName: "shield-check" },
      { to: "/casino-uden-konto", label: "Casino uden Konto", iconName: "user-x" },
      { to: "/casinoer/mobil-casinoer", label: "Mobil Casinoer", iconName: "smartphone" },
      { to: "/mobil-casino", label: "Mobil Casino Guide", iconName: "tablet-smartphone" },
      { to: "/casinoer/crypto-casino", label: "Crypto Casino", iconName: "bitcoin" },
      { to: "/casinoer/vr-casinoer", label: "VR Casinoer", iconName: "glasses" },
      { to: "/casinoer/spil-casino-for-sjov", label: "Spil Casino for Sjov", iconName: "gamepad-2" },
      { to: "/casinoer/casino-og-skat", label: "Casino og Skat", iconName: "receipt" },
    ],
  },
  {
    title: "Nye Casinoer",
    iconName: "sparkles",
    hubTo: "/nye-casinoer",
    links: [
      { to: "/nye-casinoer", label: "Nye Casinoer 2026", iconName: "calendar" },
      { to: "/nye-casinoer/dansk-licens", label: "Med Dansk Licens", iconName: "shield" },
      { to: "/nye-casinoer/uden-rofus", label: "Uden ROFUS", iconName: "shield-off" },
      { to: "/nye-casinoer/hurtig-udbetaling", label: "Hurtig Udbetaling", iconName: "zap" },
      { to: "/nye-casinoer/bonus-uden-indbetaling", label: "Bonus uden Indbetaling", iconName: "gift" },
      { to: "/nye-casinoer/trustly", label: "Med Trustly", iconName: "banknote" },
      { to: "/nye-casinoer/mitid", label: "Med MitID", iconName: "scan-line" },
      { to: "/nye-casinoer/lav-wagering", label: "Lav Wagering", iconName: "percent" },
      { to: "/nye-casinoer/vs-etablerede", label: "Nye vs Etablerede", iconName: "scale" },
    ],
  },
  {
    title: "Casinospil",
    iconName: "dices",
    hubTo: "/casinospil",
    links: [
      { to: "/casinospil", label: "Alle Casinospil", iconName: "layout-grid" },
      { to: "/casinospil/spillemaskiner", label: "Spillemaskiner", iconName: "cherry" },
      { to: "/casinospil/blackjack", label: "Blackjack", iconName: "spade" },
      { to: "/casinospil/roulette", label: "Roulette", iconName: "circle-dot" },
      { to: "/casinospil/poker", label: "Poker", iconName: "club" },
      { to: "/casinospil/baccarat", label: "Baccarat", iconName: "spade" },
      { to: "/casinospil/craps", label: "Craps", iconName: "dice-5" },
      { to: "/casinospil/online-lotteri", label: "Online Lotteri", iconName: "ticket" },
      { to: "/megaways-slots", label: "Megaways Slots", iconName: "layers" },
      { to: "/jackpot-slots", label: "Jackpot Slots", iconName: "trophy" },
      { to: "/bonus-buy-slots", label: "Bonus Buy Slots", iconName: "shopping-cart" },
    ],
  },
  {
    title: "Live Casino",
    iconName: "tv",
    hubTo: "/live-casino",
    links: [
      { to: "/live-casino", label: "Live Casino", iconName: "tv" },
      { to: "/live-casino/blackjack", label: "Live Blackjack", iconName: "spade" },
      { to: "/live-casino/roulette", label: "Live Roulette", iconName: "circle-dot" },
      { to: "/live-casino/baccarat", label: "Live Baccarat", iconName: "club" },
      { to: "/live-casino/lightning-roulette", label: "Lightning Roulette", iconName: "zap" },
      { to: "/live-casino/monopoly-live", label: "Monopoly Live", iconName: "building-2" },
      { to: "/live-casino/game-shows", label: "Game Shows", iconName: "tv" },
      { to: "/live-casino/crazy-time", label: "Crazy Time", iconName: "clock" },
      { to: "/live-casino/dream-catcher", label: "Dream Catcher", iconName: "target" },
      { to: "/live-casino/deal-or-no-deal", label: "Deal or No Deal", iconName: "briefcase" },
      { to: "/live-casino/strategi", label: "Live Casino Strategi", iconName: "brain" },
      { to: "/live-casino/udbydere", label: "Live Casino Udbydere", iconName: "users" },
    ],
  },
  {
    title: "Casino Bonus",
    iconName: "gift",
    hubTo: "/casino-bonus",
    links: [
      { to: "/casino-bonus", label: "Casino Bonus", iconName: "gift" },
      { to: "/velkomstbonus", label: "Velkomstbonus", iconName: "party-popper" },
      { to: "/free-spins", label: "Free Spins", iconName: "rotate-cw" },
      { to: "/bonus-uden-indbetaling", label: "Bonus uden Indbetaling", iconName: "gift" },
      { to: "/no-sticky-bonus", label: "No-Sticky Bonus", iconName: "unlink" },
      { to: "/sticky-bonus", label: "Sticky Bonus", iconName: "link" },
      { to: "/cashback-bonus", label: "Cashback Bonus", iconName: "undo-2" },
      { to: "/reload-bonus", label: "Reload Bonus", iconName: "refresh-cw" },
      { to: "/free-spins-i-dag", label: "Free Spins i Dag", iconName: "calendar-check" },
      { to: "/omsaetningskrav", label: "Omsætningskrav", iconName: "calculator" },
      { to: "/indskudsbonus", label: "Indskudsbonus", iconName: "piggy-bank" },
      { to: "/bonus-uden-omsaetningskrav", label: "Bonus uden Omsætningskrav", iconName: "circle-check" },
    ],
  },
  {
    title: "Betalingsmetoder",
    iconName: "creditCard",
    hubTo: "/betalingsmetoder",
    links: [
      { to: "/betalingsmetoder", label: "Alle Betalingsmetoder", iconName: "credit-card" },
      { to: "/betalingsmetoder/apple-pay", label: "Apple Pay", iconName: "apple" },
      { to: "/betalingsmetoder/mobilepay", label: "MobilePay", iconName: "smartphone" },
      { to: "/betalingsmetoder/paypal", label: "PayPal", iconName: "wallet" },
      { to: "/betalingsmetoder/skrill", label: "Skrill", iconName: "credit-card" },
      { to: "/betalingsmetoder/trustly", label: "Trustly", iconName: "banknote" },
      { to: "/betalingsmetoder/zimpler", label: "Zimpler", iconName: "send" },
      { to: "/betalingsmetoder/paysafecard", label: "Paysafecard", iconName: "shield" },
      { to: "/betalingsmetoder/bankoverforsler", label: "Bankoverførsel", iconName: "landmark" },
      { to: "/betalingsmetoder/visa-mastercard", label: "Visa / Mastercard", iconName: "credit-card" },
      { to: "/betalingsmetoder/revolut", label: "Revolut", iconName: "circle-dollar-sign" },
    ],
  },
  {
    title: "Spiludviklere",
    iconName: "gamepad2",
    hubTo: "/spiludviklere",
    links: [
      { to: "/spiludviklere", label: "Alle Spiludviklere", iconName: "layout-grid" },
      { to: "/spiludviklere/netent", label: "NetEnt", iconName: "monitor-play" },
      { to: "/spiludviklere/pragmatic-play", label: "Pragmatic Play", iconName: "clapperboard" },
      { to: "/spiludviklere/evolution-gaming", label: "Evolution Gaming", iconName: "video" },
      { to: "/spiludviklere/relax-gaming", label: "Relax Gaming", iconName: "coffee" },
      { to: "/spiludviklere/play-n-go", label: "Play'n GO", iconName: "play" },
      { to: "/spiludviklere/hacksaw-gaming", label: "Hacksaw Gaming", iconName: "axe" },
      { to: "/spiludviklere/nolimit-city", label: "Nolimit City", iconName: "flame" },
      { to: "/spiludviklere/elk-studios", label: "ELK Studios", iconName: "mountain" },
      { to: "/spiludviklere/yggdrasil", label: "Yggdrasil", iconName: "tree-pine" },
      { to: "/spiludviklere/microgaming", label: "Microgaming", iconName: "cpu" },
      { to: "/spiludviklere/red-tiger", label: "Red Tiger", iconName: "cat" },
      { to: "/spiludviklere/big-time-gaming", label: "Big Time Gaming", iconName: "clock" },
      { to: "/spiludviklere/thunderkick", label: "Thunderkick", iconName: "cloud-lightning" },
      { to: "/spiludviklere/blueprint-gaming", label: "Blueprint Gaming", iconName: "pencil-ruler" },
      { to: "/spiludviklere/push-gaming", label: "Push Gaming", iconName: "rocket" },
      { to: "/spiludviklere/quickspin", label: "Quickspin", iconName: "rotate-cw" },
      { to: "/spiludviklere/isoftbet", label: "iSoftBet", iconName: "code" },
      { to: "/spiludviklere/betsoft", label: "Betsoft", iconName: "box" },
      { to: "/spiludviklere/wazdan", label: "Wazdan", iconName: "wand" },
      { to: "/spiludviklere/endorphina", label: "Endorphina", iconName: "smile" },
      { to: "/spiludviklere/stakelogic", label: "Stakelogic", iconName: "bar-chart" },
      { to: "/spiludviklere/booming-games", label: "Booming Games", iconName: "bomb" },
    ],
  },
  {
    title: "Casino Anmeldelser",
    iconName: "star",
    hubTo: "/casino-anmeldelser",
    links: [
      { to: "/casino-anmeldelser", label: "Alle Anmeldelser", iconName: "layout-grid" },
      { to: "/casino-anmeldelser/spildansknu", label: "SpilDanskNu", iconName: "star" },
      { to: "/casino-anmeldelser/spilleautomaten", label: "Spilleautomaten", iconName: "star" },
      { to: "/casino-anmeldelser/betinia", label: "Betinia", iconName: "star" },
      { to: "/casino-anmeldelser/campobet", label: "Campobet", iconName: "star" },
      { to: "/casino-anmeldelser/swift-casino", label: "Swift Casino", iconName: "star" },
      { to: "/casino-anmeldelser/luna-casino", label: "Luna Casino", iconName: "star" },
      { to: "/casino-anmeldelser/playkasino", label: "PlayKasino", iconName: "star" },
      { to: "/casino-anmeldelser/leovegas", label: "LeoVegas", iconName: "star" },
      { to: "/casino-anmeldelser/mr-green", label: "Mr Green", iconName: "star" },
      { to: "/casino-anmeldelser/unibet", label: "Unibet", iconName: "star" },
      { to: "/casino-anmeldelser/bet365", label: "bet365", iconName: "star" },
      { to: "/casino-anmeldelser/comeon", label: "ComeOn", iconName: "star" },
      { to: "/casino-anmeldelser/danske-spil", label: "Danske Spil", iconName: "star" },
      { to: "/casino-anmeldelser/getlucky", label: "GetLucky", iconName: "star" },
      { to: "/casino-anmeldelser/888-casino", label: "888 Casino", iconName: "star" },
      { to: "/casino-anmeldelser/betano", label: "Betano", iconName: "star" },
      { to: "/casino-anmeldelser/royal-casino", label: "Royal Casino", iconName: "star" },
      { to: "/casino-anmeldelser/mr-vegas", label: "Mr Vegas", iconName: "star" },
      { to: "/casino-anmeldelser/videoslots", label: "Videoslots", iconName: "star" },
      { to: "/casino-anmeldelser/nordicbet", label: "NordicBet", iconName: "star" },
      { to: "/casino-anmeldelser/spilnu", label: "Spilnu", iconName: "star" },
      { to: "/casino-anmeldelser/stake-casino", label: "Stake Casino", iconName: "star" },
    ],
  },
];
