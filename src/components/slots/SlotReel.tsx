...
export const SlotReel = React.memo(function SlotReel({
  ...
  const totalSymbolHeight = symbolSize + symbolGap;
  const viewportHeight = 3 * symbolSize + 2 * symbolGap;
  ...
  }, [isSpinning, spinState, buildReelStrip, totalSymbolHeight, spinLoopMs, applyOffset]);
  ...
  }, [shouldSlowDown, spinState, teaseMode, isActiveTeaseReel, delay, onReelStop, buildReelStrip, totalSymbolHeight, reelSlowdownMs, applyOffset]);
    ...
    >
      {isAnimating ? (
        <div
          ...
          style={{ gap: `${symbolGap}px`, willChange: "transform, filter" }}
        >
          {spinningStripJsx}
        </div>
      ) : (
        <div className="absolute left-0 right-0 flex flex-col" style={{ gap: `${symbolGap}px` }}>
