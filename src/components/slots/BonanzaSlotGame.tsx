...
export function BonanzaSlotGame({ gameId = "fedesvin-bonanza", isMobile = false }: BonanzaSlotGameProps) {
  ...
  const processTumbleSteps = useCallback(async (steps: BonanzaTumbleStep[]) => {
    ...
    if (lastStepWithBombs?.multiplierBombs?.length) {
      ...
      const targetEl = isMobile ? document.getElementById("bonanza-mult-target") : null;
  ...
  return (
    <div className={cn("flex flex-col items-center relative", isMobile ? "gap-2 w-full" : "gap-4")} style={isMobile ? undefined : { width: totalLayoutWidth, maxWidth: "100%" }}>
      ...
      >
        ...
        {/* Live chat panel — right side (desktop only), spaced far from grid */}
        {!isMobile && (
          <div
            className="shrink-0 overflow-hidden ml-56"
            style={{
        ...
        <div className="w-full flex justify-center">
          <div className="flex items-baseline gap-2">
            <span
              ...
              style={{ textShadow: "0 2px 6px rgba(0,0,0,0.9), 0 0 12px rgba(236,72,153,0.6)" }}
            >
              Resterende spins
            </span>
            <AnimatedSpinCounter
