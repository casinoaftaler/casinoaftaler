    ...
    <Card className="rounded-2xl border-primary/10 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary/10">
      <CardContent className="p-4 space-y-3">
        ...
        <div className="space-y-0">
          <StatRow iconName="gift" label="Antal Bonusser" value={s.totalBonuses} />
          <StatRow iconName="unlock" label="Åbnede" value={`${s.openedBonuses} / ${s.totalBonuses}`} />
          <StatRow iconName="wallet" label="Start Balance" value={`${s.startBalance.toLocaleString('da-DK')} kr`} />
          <StatRow iconName="target" label="Target Balance" value={`${s.targetBalance.toLocaleString('da-DK')} kr`} />
          <StatRow iconName="trophy" label="Winnings" value={`${s.totalWinnings.toLocaleString('da-DK')} kr`} highlight={s.totalWinnings > 0} />
          <StatRow iconName="wallet" label="End Balance" value={s.endBalance !== null ? `${s.endBalance.toLocaleString('da-DK')} kr` : '—'} highlight={!!s.endBalance} />
          {s.endBalance !== null && (
            <div className={`flex items-center justify-between py-1.5 border-b border-border/20 last:border-0 group/row transition-all duration-[180ms] hover:bg-muted/20 rounded-md px-1`}>
              <span className="text-xs text-muted-foreground flex items-center gap-1.5">
