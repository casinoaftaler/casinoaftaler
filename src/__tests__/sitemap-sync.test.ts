...
function expectedLastmod(lastmod: string) {
  return `${lastmod}T12:00:00+01:00`;
}
  ...
  it('matches seoRoutes 1:1', () => {
    ...
    const expected = new Map(
      seoRoutes.map((route) => {
        ...
        return [loc, {
          lastmod: expectedLastmod(route.lastmod!),
    ...
    for (const [loc, route] of expected.entries()) {
      ...
      if (
        ...
        mismatches.push(`diff:${loc}:${JSON.stringify(route)}!=${JSON.stringify(sitemapEntry)}`);
      }
    }
